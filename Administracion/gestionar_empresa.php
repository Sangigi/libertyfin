...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($plan === 'premium' && (!isset($empresa['plan']) || $empresa['plan'] !== 'premium')) {
        ...
        // Registrar activación del plan premium
        $precio_sin_iva = 1299;
            ...
            if ($stmt->execute()) {
                ...
                if (isset($precio_sin_iva) && isset($precio_con_iva) && $empresa['plan'] != $plan) {
                    $sql_insert_plan = "INSERT INTO activaciones_plan 
                                       ...
                                       VALUES (?, ?, ?, ?, ?, NOW(), ?)";
                    $stmt_plan = $conn->prepare($sql_insert_plan);
                    $usuario = $_SESSION['usuario_nombre'] ?? 'admin';
                    $plan_anterior = $empresa['plan'] ?? null;
                    $stmt_plan->bind_param("issdds", $id_empresa, $plan_anterior, $plan, $precio_sin_iva, $precio_con_iva, $usuario);
                    $stmt_plan->execute();
                    $stmt_plan->close();
                }
            ...
            if ($stmt->execute()) {
                ...
                if ($plan != 'prueba') {
                    ...
                    if (isset($precios_planes[$plan])) {
                        ...
                        $stmt_plan->bind_param("isdds", $id_empresa, $plan, $precio_sin_iva, $precio_con_iva, $usuario);
                        $stmt_plan->execute();
                        $stmt_plan->close();
                    }
                }
...
<body>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" id="plan" name="plan" required>
                                            ...
                                            <option value="prueba" data-precio="0" data-precio-iva="0" <?php echo (isset($empresa['plan']) && $empresa['plan'] == 'prueba') ? 'selected' : ''; ?>>
                                                Prueba - $0 MXN
                                            </option>
                                            <option value="basico" data-precio="299" data-precio-iva="346.84" <?php echo (isset($empresa['plan']) && $empresa['plan'] == 'basico') ? 'selected' : ''; ?>>
                                                Básico - $299 MXN + IVA = $346.84 MXN
                                            </option>
                                            <option value="starter" data-precio="599" data-precio-iva="694.84" <?php echo (isset($empresa['plan']) && $empresa['plan'] == 'starter') ? 'selected' : ''; ?>>
                                                Starter - $599 MXN + IVA = $694.84 MXN
                                            </option>
                                            <option value="emprendedor" data-precio="999" data-precio-iva="1158.84" <?php echo (isset($empresa['plan']) && $empresa['plan'] == 'emprendedor') ? 'selected' : ''; ?>>
                                                Emprendedor - $999 MXN + IVA = $1,158.84 MXN
                                            </option>
                                            <option value="premium" data-precio="1299" data-precio-iva="1506.84" <?php echo (isset($empresa['plan']) && $empresa['plan'] == 'premium') ? 'selected' : ''; ?>>
    ...
    <script>
        ...
        window.confirmarAgregarTimbres = function() {
            const radioSeleccionado = document.querySelector('input[name="cantidad_timbres"]:checked');
            
            if (!radioSeleccionado) {
                Swal.fire({
                    title: 'Error',
                    text: 'Por favor seleccione un paquete de timbres',
                    icon: 'error',
            ...
            const cantidad = radioSeleccionado.value;
            const precioIVA = radioSeleccionado.dataset.precioIva;
            const totalTimbres = document.getElementById('totalTimbres').textContent;

            Swal.fire({
                title: '¿Comprar paquete de timbres?',
                html: `Paquete seleccionado: <strong>${cantidad} CFDI</strong><br><br>
                       Total a pagar: <strong class="text-primary">${totalTimbres} MXN</strong><br><br>
                       Timbres disponibles actuales: <strong><?php echo $empresa['timbres_disponibles']; ?></strong><br>
                       Total después de la compra: <strong><?php echo $empresa['timbres_disponibles'] + 0; ?> + ${cantidad} = <?php echo $empresa['timbres_disponibles']; ?> + ${cantidad}</strong>`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#6c757d',
        ...
        // Función para confirmar agregar sucursales
        window.confirmarAgregarSucursales = function() {
            const numSucursales = document.getElementById('num_sucursales').value;
            const totalSucursales = document.getElementById('totalSucursales').textContent;
            
            Swal.fire({
                title: '¿Agregar sucursal(es) extra?',
                html: `Se agregarán <strong>${numSucursales} sucursal(es) extra(s)</strong>.<br><br>
                       Total a pagar: <strong class="text-success">${totalSucursales} MXN</strong>`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#6c757d',
                ...
                if (result.isConfirmed) {
                    document.getElementById('formSucursales').submit();
                }
            });
    ...
    `;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const planSubtotal = document.getElementById('planSubtotal');
            const planIVA = document.getElementById('planIVA');
            const planTotal = document.getElementById('planTotal');
            const empresaForm = document.getElementById('empresaForm');
            const radioTimbres = document.querySelectorAll('.precio-timbres');
            const resumenTimbres = document.getElementById('resumenTimbres');
            const paqueteSeleccionado = document.getElementById('paqueteSeleccionado');
            const precioSinIVA = document.getElementById('precioSinIVA');
            const ivaTimbres = document.getElementById('ivaTimbres');
            const totalTimbres = document.getElementById('totalTimbres');
            const numSucursales = document.getElementById('num_sucursales');
            ...
            // Mostrar información de precios del plan seleccionado
            function actualizarPrecioPlan() {
                if (planSelect && planSelect.value) {
                    const selectedOption = planSelect.options[planSelect.selectedIndex];
                    const precio = parseFloat(selectedOption.dataset.precio || 0);
                    const precioIVA = parseFloat(selectedOption.dataset.precioIva || 0);
                    
                    planSubtotal.textContent = '$' + precio.toFixed(2) + ' MXN';
                    planIVA.textContent = '$' + (precio * 0.16).toFixed(2) + ' MXN';
                    planTotal.textContent = '$' + precioIVA.toFixed(2) + ' MXN';
                    planPriceInfo.style.display = 'block';
                } else {
                    planPriceInfo.style.display = 'none';
                }
            }

            if (planSelect) {
                planSelect.addEventListener('change', actualizarPrecioPlan);
                actualizarPrecioPlan(); // Mostrar precio inicial si hay un plan seleccionado
            }

            // Manejar selección de timbres en el modal
            if (radioTimbres.length > 0) {
                radioTimbres.forEach(radio => {
                    radio.addEventListener('change', function() {
                        if (this.checked) {
                            const cantidad = this.value;
                            const precio = parseFloat(this.dataset.precio);
                            const precioIVA = parseFloat(this.dataset.precioIva);
                            
                            paqueteSeleccionado.textContent = cantidad + ' CFDI';
                            precioSinIVA.textContent = '$' + precio.toFixed(2);
                            ivaTimbres.textContent = '$' + (precio * 0.16).toFixed(2);
                            totalTimbres.textContent = '$' + precioIVA.toFixed(2);
                            resumenTimbres.style.display = 'block';
                        }
                    });
                });
            }

            // Manejar cálculo de sucursales extra
            if (numSucursales) {
                numSucursales.addEventListener('input', function() {
                    const cantidad = parseInt(this.value) || 1;
                    const precioUnitario = 499;
                    const subtotal = cantidad * precioUnitario;
                    const iva = subtotal * 0.16;
                    const total = subtotal + iva;

                    document.getElementById('subtotalSucursales').textContent = '$' + subtotal.toFixed(2);
                    document.getElementById('ivaSucursales').textContent = '$' + iva.toFixed(2);
                    document.getElementById('totalSucursales').textContent = '$' + total.toFixed(2);
                });
            }
