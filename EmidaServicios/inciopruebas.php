...
try {
    ...
    // =============================================
    // INCLUIR Y PROCESAR LA RESPUESTA DE EMIDA VÍA PROXY
    // =============================================
    $servicios_emida = [];
    $error_emida = '';

    // Obtener terminal_id y clerk_id de la sesión o configuración
    $terminal_id = $_SESSION['terminal_id'] ?? '9478382';
    $clerk_id = $_SESSION['clerk_id'] ?? 'UFJ45M';
...
<body>
    ...
    <script>
        ...
        async function procesarVentaConTimeout(datosVenta) {
            ...
            var endpoint = 'proxy_transaction.php';
            ...
            if (timeoutOcurrido || (respuestaInicial && respuestaInicial.requires_lookup === true)) {
                ...
                var saldoResponse = await fetch('proxy_get_balance.php?_=' + new Date().getTime());
                ...
                for (var intento = 1; intento <= config.numReintentos; intento++) {
                    ...
                    try {
                        var consultaResponse = await fetch('proxy_lookup.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                ...
                var saldoFinalResponse = await fetch('proxy_get_balance.php?_=' + new Date().getTime());
        ...
        function obtenerSaldoEmida(mostrarNotificacionFlag) {
            ...
            fetch('proxy_get_balance.php?_=' + new Date().getTime())
        ...
        function verificarEstadoNotificaciones() {
            ...
            fetch('proxy_notifications.php?action=status')
                .then(function(response) {
                    return response.json();
                })
                .then(function(data) {
        ...
        function enviarNotificacionPago(datosVenta) {
            ...
            fetch('proxy_notifications.php?action=send', {
                    method: 'POST',
                    headers: {
