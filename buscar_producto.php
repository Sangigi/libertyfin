...
try {
    ...
    // MODIFICACIÓN: Buscar por la columna 'codigo' en lugar de 'codigo_barras'
    $sql = "
        SELECT 
            p.id,
            p.codigo,
        ...
        WHERE p.codigo = ?  -- CAMBIO: Usar columna 'codigo' en lugar de 'codigo_barras'
        AND p.activo = 1
    ...
    if ($stmt) {
        $stmt->bind_param("is", $sucursal_id, $codigo_barras);
        $stmt->execute();
        ...
        if ($producto) {
            // OBTENER LA IMAGEN PRINCIPAL DE LA TABLA producto_imagenes
